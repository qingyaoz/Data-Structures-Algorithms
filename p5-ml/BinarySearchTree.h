...
private:
    ...
    static int size_impl(const Node *node) {
        ...
        else return 1 + size_impl(node->left) +size_impl(node->right);
    }
    ...
    static bool check_sorting_invariant_impl(const Node *node, Compare less) {
        ...
        //      Node *min = min_element_impl(node->right);
        //      Node *max = max_element_impl(node->left);
        
              if(node -> left && 
                    (less(node->datum, max_element_impl(node->left)->datum) ||
                    (!less(node->datum,max_element_impl(node->left)->datum) &&
