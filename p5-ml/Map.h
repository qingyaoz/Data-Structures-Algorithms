...
public:
  ...
  // copy, assign, and destroy Maps.
    ...
    // copy constructor
    Map(const Map &other) : btree(other.btree) {}
  ...
  //       using "Value_type()".
    Iterator find(const Key_type& k) const {
      return btree.find(Pair_type(k, Value_type()));
    }
  ...
  // HINT: http://www.cplusplus.com/reference/map/map/operator[]/
    Value_type& operator[](const Key_type& k) {
      // Iterator finded = find(k); 
        if (find(k) == btree.end()) {
          Pair_type pair = std::make_pair(k, Value_type());
          return (btree.insert(pair))->second;
        }else{
  ...
  //           the value true.
    std::pair<Iterator, bool> insert(const Pair_type &val) {
        // Iterator finded = btree.find(val);
        if (btree.find(val) == btree.end()) {
          // Iterator inserted = btree.insert(val);
          return std::pair<Iterator, bool>(btree.insert(val), true);
        } else {
          return std::pair<Iterator, bool>(btree.find(val), false);
        }
    }
...
private:
  // Add a BinarySearchTree private member HERE.
    BinarySearchTree<Pair_type, PairComp> btree;
};
